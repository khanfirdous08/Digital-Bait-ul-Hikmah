...
export default function QuantumMerge() {
  ...
  const spawnFX = useCallback((x: number, y: number, level: number) => {
    ...
    if (g.sparks.length > g.maxSparks)
      g.sparks.splice(0, g.sparks.length - g.maxSparks + 20);
    ...
    for (let i = 0; i < count; i++) {
      ...
      g.sparks.push({
        ...
        life: 1,
        maxLife: rnd(0.4, 0.9 + level * 0.08),
        color,
        glow,
        size: rnd(2, 5 + level * 0.4),
      });
    }
    ...
    for (let ring = 0; ring < rings; ring++) {
      g.sparks.push({
        ...
        vy: 0,
        life: 1,
        maxLife: 0.5 + ring * 0.12,
        color,
        glow,
        size: 0,
        radius: 0,
        maxRadius: CUBE_SIZES[level] * (1.8 + ring * 1.2),
      });
    }
    if (!mobile) {
    ...
    for (let i = 0; i < (mobile ? 5 : 10); i++) {
      ...
      g.sparks.push({
        ...
        x: x + rnd(-12, 12),
        y: y + rnd(-12, 12),
        vx: Math.cos(angle) * rnd(1, 4),
        vy: Math.sin(angle) * rnd(1, 4) - 1,
        life: 1,
        maxLife: rnd(0.6, 1.2),
        color: "#fff",
  ...
  const doMerge = useCallback(
    (bodyA: CubeBody, bodyB: CubeBody) => {
      ...
      if (g.score > hs) {
        store.update((d) => {
          d.highScore = g.score;
          d.maxLevelReached = Math.max(d.maxLevelReached, newLevel);
        });
        ...
        for (const pair of event.pairs) {
          ...
          if (
            ...
            setTimeout(() => {
              ...
              const ca = g2.cubes.find((c) => c.id === idA) as
                | CubeBody
                | undefined;
              const cb = g2.cubes.find((c) => c.id === idB) as
                | CubeBody
                | undefined;
              if (ca && cb) {
                playSound(500, 0.1);
  ...
  const dropCube = useCallback(() => {
    ...
    const x = Math.max(
      size + WALL_T + 2,
      Math.min(GW - size - WALL_T - 2, g.aimX),
    );
    ...
    setTimeout(() => {
      if (gRef.current) gRef.current.canDrop = true;
    }, 200);
  ...
  const doContinue = useCallback(() => {
    const g = gRef.current;
    if (!g || g.continueUsed) return;
    g.continueUsed = true;
    // Remove cubes in top 40% of play area
    const cutoff = GH * 0.4;
    const toRemove = g.cubes.filter(
      (c) => c.position.y - CUBE_SIZES[c.cubeLevel ?? 0] < cutoff,
    );
  ...
  const drawBg = useCallback(
    (ctx: CanvasRenderingContext2D, t: number, bgPulse: number) => {
      ...
      ctx.lineWidth = 1;
      for (
        let x = (scroll % gridSize) - gridSize;
        x < GW + gridSize;
        x += gridSize
      ) {
        ctx.beginPath();
  ...
  const drawTrail = useCallback(
    (
      ...
      if (
        speed > 2 &&
        (Math.abs(cube.position.x - lastX) > 3 ||
          Math.abs(cube.position.y - lastY) > 3)
      ) {
  ...
  const drawSparks = useCallback(
    (ctx: CanvasRenderingContext2D, dt: number, isMobile: boolean) => {
      const g = gRef.current;
      if (!g) return;
      ...
      for (const sp of g.sparks) {
        ...
        if (sp.type === "ring") {
          ...
          ctx.shadowColor = sp.glow;
          ctx.shadowBlur = shadowCap * 0.6;
          ctx.strokeStyle = sp.color;
          ctx.lineWidth = sp.size * sp.life;
          ctx.beginPath();
          ctx.moveTo(sp.x, sp.y);
          ctx.lineTo(
            sp.x - Math.cos(sp.angle ?? 0) * len,
            sp.y - Math.sin(sp.angle ?? 0) * len,
          );
          ctx.stroke();
        } else {
          sp.x += sp.vx;
  ...
  const drawFloats = useCallback(
    (ctx: CanvasRenderingContext2D, dt: number) => {
      ...
      for (const ft of g.floatTexts) {
        ...
        const sc =
          ft.scale *
          (ft.life > 0.85 ? easeOutBack(1 - (ft.life - 0.85) / 0.15) : 1);
        ctx.save();
        ctx.globalAlpha = a;
  ...
  const getMergeScale = useCallback(
    (id: number, anims: MergeAnim[], now: number) => {
  ...
  const drawHUD = useCallback(
    (ctx: CanvasRenderingContext2D, g: GameState, t: number) => {
      ...
      ctx.beginPath();
      ctx.moveTo(0, DROP_Y);
      ctx.lineTo(GW, DROP_Y);
      ctx.stroke();
      ctx.setLineDash([]);

      // Danger zone
      ctx.fillStyle = `rgba(255,40,40,${0.05 + pulse * 0.04})`;
      ctx.fillRect(0, 0, GW, GAMEOVER_Y);
      ctx.strokeStyle = `rgba(255,60,60,${0.45 + pulse * 0.3})`;
      ctx.lineWidth = 1;
      ctx.setLineDash([6, 6]);
      ctx.lineDashOffset = -t * 28;
      ctx.beginPath();
      ctx.moveTo(0, GAMEOVER_Y);
      ctx.lineTo(GW, GAMEOVER_Y);
      ctx.stroke();
      ctx.setLineDash([]);
      ...
      ctx.fillText(
        Math.max(g.score, store.get("highScore")).toLocaleString(),
        GW - 12,
        20,
      );
      ...
      if (g.combo > 1) {
        ...
        roundRect(ctx, bx, by, bw * cb, bh, 2);
        ctx.fill();
      }
      ctx.restore();
      ...
      roundRect(ctx, GW - 44, GH - 44, 36, 36, 6);
      ctx.fill();
      ctx.shadowColor = "#00e5ff";
      ctx.shadowBlur = 8;
  ...
  useEffect(() => {
    ...
    const g: GameState = {
      ...
      paused: false,
      continueUsed: false,
      continueCountdown: CONTINUE_SECS,
      gameLevel: 0,
      shake: 0,
      shakeX: 0,
      shakeY: 0,
    ...
    function loop(now: number) {
      ...
      for (const cube of g.cubes) {
        ...
        const level = cube.cubeLevel ?? 0;
        const flashAlpha = isGO
          ? 0.5 + 0.5 * Math.sin(g.gameOverTimer * 18)
          : 1;
      ...
      if (g.phase === "playing" && !g.paused) {
        ...
        ctx.shadowColor = CUBE_GLOW[g.curLevel];
        ctx.shadowBlur = 14;
        ...
        ctx.save();
        ctx.strokeStyle = CUBE_COLORS[g.curLevel];
        ctx.globalAlpha = 0.07 + gpulse * 0.05;
        ctx.lineWidth = 1;
        ctx.setLineDash([4, 8]);
  ...
  const backFromSettings = useCallback(
    () => setOverlay(phase === "start" ? "none" : "pause"),
    [phase],
  );
  const backFromAchievements = useCallback(
    () => setOverlay(phase === "start" ? "none" : "pause"),
    [phase],
  );
  ...
  const handleStartAchievements = useCallback(
        ...
        >
          ...
          <canvas
            ...
            style={{
              ...
              boxShadow:
                "0 0 60px rgba(0,229,255,0.1), 0 0 120px rgba(0,100,255,0.05)",
              touchAction: "none",
            }}
          />
          ...
          {toasts.length > 0 && (
            <AchievementToast
              def={toasts[0]}
              onDone={() => setToasts((prev) => prev.slice(1))}
            />
          )}
        </div>
      </div>
